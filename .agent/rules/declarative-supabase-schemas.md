...
| ステップ | コマンド | 説明 |
|---------|---------|------|
| **1. スキーマ作成** | `echo "create table users(id int);" > supabase/schemas/users.sql` | テーブル定義 |
| **2. 差分生成** | `supabase db diff -f create_users` | マイグレーション作成 |
| **3. 適用** | `supabase start && supabase migration up` | ローカルDB反映 |
| **4. 確認** | 生成されたマイグレーションファイルを開いて確認 | 内容検証 |

**所要時間**: 約5分 | **前提条件**: Supabase CLI インストール済み

## 2.6. 必須制約と禁止事項（CRITICAL）

| 制約タイプ | ルール | 理由 |
|-----------|--------|------|
| **⛔ 禁止** | 本番デプロイ済みマイグレーションのリセット禁止 | データ損失リスク |
| **⛔ 禁止** | カラムを途中に挿入（必ず末尾に追加） | ビュー/列挙型の破損 |
| **⛔ 禁止** | マイグレーションファイルの直接編集・削除 | 履歴の整合性崩壊 |
| **✅ 必須** | 生成されたマイグレーションの目視確認 | 意図しない変更検出 |
| **✅ 必須** | 本番デプロイ前のバックアップ取得 | 復旧可能性の確保 |
| **⚠️ 注意** | ダウンマイグレーションは破壊的（DROP/DELETE含む） | 慎重なレビュー必須 |
...
| カテゴリ | 未サポート/制限内容 | 代替手段 |
|---------|-------------------|----------|
| **DML** | INSERT, UPDATE, DELETE | [従来のマイグレーション](https://supabase.com/docs/guides/deployment/database-migrations) |
| **ビュー** | 所有権管理、セキュリティ呼び出し元変更、マテリアライズドビュー制限、カラムタイプ変更時の再作成問題 | 手動管理または従来方式 |
| **RLS** | ALTER POLICY 文、カラム権限追跡 | 従来のマイグレーション |
| **権限** | スキーマ権限、デフォルト権限の重複GRANT | 従来のマイグレーション |
| **その他** | コメント、パーティション、パブリケーション変更、ドメイン作成 | 従来のマイグレーション |

**重要**: 上記機能が必要な場合は、宣言型スキーマと従来のバージョン管理マイグレーションを併用してください。

## 8. ベストプラクティス

| カテゴリ | 必須ルール | 理由 |
|---------|-----------|------|
| **ファイル組織** | テーブル/機能ごとに分割、明確な命名、外部キーの親を先に配置 | 可読性と保守性の向上 |
| **マイグレーション確認** | 生成後の内容を必ず目視確認、破壊的変更（DROP/DELETE）に特に注意 | 意図しない変更の防止 |
| **デプロイ手順** | ローカル→ステージング→本番の順、本番前にバックアップ取得 | リスク最小化と復旧可能性 |
| **バージョン管理** | `supabase/schemas/` と `supabase/migrations/` を Git 管理、マイグレーションファイルは削除・編集禁止 | 変更履歴の完全性と追跡性 |
| **Drizzle統合** | `src/db/schema/*.ts` に配置、`casing: 'snake_case'` 統一、命名規則の自動検証 | 一貫性と自動化による品質確保 |

## 9. トラブルシューティング

| 問題 | 確認項目 | 解決方法 |
|------|---------|----------|
| **マイグレーション生成されない** | スキーマファイル構文エラー、ローカルDB起動状態、既存マイグレーション適用状態 | `supabase start` 実行、`supabase migration up` で既存分を適用 |
| **意図しない差分が生成** | スキーマファイルの記述順序、カラム追加位置（末尾か）、既存マイグレーションとの競合 | カラムは必ず末尾に追加、ファイル順序を `config.toml` で明示 |
| **デプロイエラー** | ローカル/リモートのマイグレーション状態、ネットワーク接続、権限 | `supabase db remote commit` でリモート変更取り込み、権限確認 |
